...
func (cfg *config) makeClient(to []int) *Clerk {
	...
	ck := MakeMockRpcClerk(random_handles(ends))
...
func (cfg *config) StartServer(i int) {
	...
	cfg.servers[i] = StartTestServer(ends, uint64(i), cfg.saved[i])

	kvsvc := mockrpc.MakeService(cfg.servers[i])
	rfsvc := mockrpc.MakeService(cfg.servers[i].rf)
