...
func TestFailNoAgree(t *testing.T) {
	...
	index, _, ok := cfg.rafts[leader].Start([]byte(strconv.Itoa(20)))
	if ok != true {
	...
	// or perhaps
	leader2 := cfg.checkOneLeader()
	index2, _, ok2 := cfg.rafts[leader2].Start([]byte(strconv.Itoa(30)))
	if ok2 == false {
...
loop:
	for try := 0; try < 5; try++ {
		...
		leader := cfg.checkOneLeader()
		_, term, ok := cfg.rafts[leader].Start([]byte(strconv.Itoa(1)))
		if !ok {
		...
		for ii := 0; ii < iters; ii++ {
			...
			go func(i int) {
				defer wg.Done()
				iii, term1, ok := cfg.rafts[leader].Start([]byte(strconv.Itoa((100 + i))))
				if term1 != term {
					return
				}
				if ok != true {
		...
		cmds := [][]byte{}
		for index := range is {
			cmd := cfg.wait(int(index), servers, int(term))
			if ix, ok := cmd.([]byte); ok {
				if string(ix) == "-1" {
		...
		for ii := 0; ii < iters; ii++ {
			x := []byte(strconv.Itoa(100 + ii))
			ok := false
			for j := 0; j < len(cmds); j++ {
				if SameByteSlice(cmds[j], x) {
					ok = true
				}
			}
			if ok == false {
...
func TestRejoin(t *testing.T) {
	...
	cfg.rafts[leader1].Start([]byte(strconv.Itoa(102)))
	cfg.rafts[leader1].Start([]byte(strconv.Itoa(103)))
	cfg.rafts[leader1].Start([]byte(strconv.Itoa(104)))
...
func TestBackup(t *testing.T) {
	...
	for i := 0; i < 50; i++ {
		cfg.rafts[leader1].Start([]byte(strconv.Itoa(rand.Int())))
	}
	...
	for i := 0; i < 50; i++ {
		cfg.rafts[leader2].Start([]byte(strconv.Itoa(rand.Int())))
	}
...
loop:
	for try := 0; try < 5; try++ {
		...
		iters := 10
		starti, term, ok := cfg.rafts[leader].Start([]byte(strconv.Itoa(1)))
		if !ok {
			// leader moved on really quickly
			continue
		}
		cmds := [][]byte{}
		for i := 1; i < iters+2; i++ {
			x := []byte(strconv.Itoa(int(rand.Int31())))
			cmds = append(cmds, x)
		...
		for i := 1; i < iters+1; i++ {
			cmd := cfg.wait(int(starti)+i, servers, int(term))
			if ix, ok := cmd.([]byte); ok == false || SameByteSlice(ix, cmds[i-1]) == false {
				if string(ix) == "-1" {
...
func TestFigure8(t *testing.T) {
	...
	for iters := 0; iters < 20; iters++ {
		...
		for i := 0; i < servers; i++ {
			if cfg.rafts[i] != nil {
				_, _, ok := cfg.rafts[i].Start([]byte(strconv.Itoa(rand.Int())))
				if ok {
					leader = i
				}
			}
...
func TestFigure8Unreliable(t *testing.T) {
	...
	for iters := 0; iters < 1000; iters++ {
		...
		leader := -1
		for i := 0; i < servers; i++ {
			_, _, ok := cfg.rafts[i].Start([]byte(strconv.Itoa(rand.Int() % 10000)))
...
func internalChurn(t *testing.T, unreliable bool) {
	...
	cfn := func(me int, ch chan [][]byte) {
		var ret [][]byte
		ret = nil
		defer func() { ch <- ret }()
		values := [][]byte{}
		for atomic.LoadInt32(&stop) == 0 {
			x := []byte(strconv.Itoa(rand.Int()))
			index := -1
			ok := false
			for i := 0; i < servers; i++ {
			...
			if ok {
				...
				for _, to := range []int{10, 20, 50, 100, 200} {
					...
					if nd > 0 {
						if xx, ok := cmd.([]byte); ok {
							if SameByteSlice(x, xx) {
